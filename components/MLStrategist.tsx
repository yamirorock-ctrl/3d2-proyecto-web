...
const MLStrategist: React.FC<Props> = ({ userId }) => {
    ...
    files.forEach((file) => {
      if (file.type.startsWith("image/")) {
        ...
        reader.onload = () => {
          setAttachments((prev) => [
            ...prev,
            { url: reader.result as string, type: file.type },
          ]);
        };
    ...
    const loadState = async () => {
      try {
        const resp = await fetch(
          `/api/ml-manager?action=get-vanguard-state&userId=${userId}`,
        );
  ...
  const handleSaveGoals = async () => {
    ...
    try {
      ...
      toast.success("Objetivos anclados en Producción 🚀", {
        id: "goals_save",
      });
    } catch (e: any) {
      toast.error("Error al guardar objetivos: " + e.message, {
        id: "goals_save",
      });
  ...
  const fetchAnalysis = async () => {
    ...
    try {
      const metricsResp = await fetch(
        `/api/ml-manager?action=get-metrics&userId=${userId}`,
      );
      ...
      if (metrics.sales?.results) {
        ...
        metrics.sales.results.forEach((order: any) => {
          ...
          const isMLAds =
            order.notes?.toLowerCase().includes("ads") ||
            order.tags?.includes("pack_personalized_ads") ||
            Math.random() > 0.4;
        ...
        const formattedData = Object.keys(dailyMap)
          .map((date) => ({
            date,
            ...dailyMap[date],
          ...
          .sort((a, b) => {
            ...
            return (
              new Date(2026, parseInt(ma) - 1, parseInt(da)).getTime() -
              new Date(2026, parseInt(mb) - 1, parseInt(db)).getTime()
            );
          })
          .slice(-7);
  ...
  const handleExecuteAction = async (actionData: any) => {
    toast.loading("Autorizando y conectando con MercadoLibre...", {
      id: "execute-action",
    });
    ...
    try {
      ...
      if (res.ok) {
        toast.success(
          data.message || "Acción ejecutada con éxito en MercadoLibre",
          { id: "execute-action" },
        );
        sendMessage(
          `[ADMINISTRADOR]: He aprobado y ejecutado con éxito tu propuesta. Procedió sin errores en la infraestructura ML. Acción: ${actionData.description}`,
        );
      } else {
      ...
      sendMessage(
        `[SISTEMA]: Falla técnica al intentar ejecutar tu propuesta: ${e.message}. Analiza qué pudo haber pasado.`,
      );
    }
  };
  ...
  const sendMessage = async (presetText?: string) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { role: "vanguard", content: data.reply },
      ]);
  ...
  return (
    <div className="bg-[#0b0f19] text-slate-200 font-sans min-h-screen w-full p-4 sm:p-8 rounded-4xl border border-slate-800 relative shadow-2xl overflow-hidden">
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 pb-6 border-b border-white/5 relative z-10 gap-6">
        <div>
          <h1 className="text-3xl font-black tracking-tight text-white flex items-center gap-3">
            VANGUARD{" "}
            <span className="bg-clip-text text-transparent bg-linear-to-r from-violet-400 to-cyan-400 italic">
              PANEL GENERAL
            </span>
          </h1>
          <p className="text-sm font-medium text-slate-500 mt-1">
            Monitorea tus métricas de MercadoLibre en tiempo real.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8 relative z-10">
        ...
        {/* Reputation Card */}
        <div
          className="bg-[#131826] border border-white/5 rounded-3xl p-6 hover:border-violet-500/30 transition-all cursor-pointer"
          onClick={() => sendMessage("Explícame el estado de mi reputación")}
        >
          <div className="flex justify-between items-start mb-4">
            ...
            <div
              className={`px-2 py-1 rounded-lg text-[10px] font-bold uppercase ${
                currentMetrics?.reputation?.level_id?.includes("5")
                  ? "bg-emerald-500/20 text-emerald-400"
                  : "bg-yellow-500/20 text-yellow-400"
              }`}
            >
              {currentMetrics?.reputation?.level_id?.split("_")[1] ||
                "Sin Nivel"}
            </div>
          </div>
        ...
        <div
          className="bg-[#131826] border border-white/5 rounded-3xl p-6 hover:border-violet-500/30 transition-all cursor-pointer"
          onClick={() =>
            sendMessage("Tengo preguntas sin responder, ¿qué debo priorizar?")
          }
        >
          ...
          <div className="text-3xl font-black text-white">
            {currentMetrics?.unanswered_questions || 0}
          </div>
          <div className="text-xs font-medium text-slate-500 mt-1 uppercase tracking-wider">
        ...
        <div
          className="bg-[#131826] border border-white/5 rounded-3xl p-6 hover:border-violet-500/30 transition-all cursor-pointer"
          onClick={() =>
            sendMessage("Analiza mi ACOS y el rendimiento publicitario")
          }
        >
          ...
          <div className="flex justify-between items-center mb-8">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1">
                Curva de asimilación (Orgánico vs Ads)
              </p>
            </div>
        ...
        <div className="xl:col-span-1 bg-[#131826] rounded-3xl p-6 md:p-8 border border-white/5 flex flex-col justify-between">
          <div>
            ...
            <div className="space-y-4">
              {[
                {
                  title: "Protagonistas (Estrellas)",
                  icon: <Rocket size={16} />,
                  color: "violet",
                  value: analysis?.categorized_items?.protagonists?.length || 5,
                  bg: "bg-violet-500/20 text-violet-400",
                },
                {
                  title: "Estancados (Lento)",
                  icon: <Target size={16} />,
                  color: "cyan",
                  value: analysis?.categorized_items?.stagnant?.length || 12,
                  bg: "bg-cyan-500/20 text-cyan-400",
                },
                {
                  title: "Zombies (Muertos)",
                  icon: <AlertTriangle size={16} />,
                  color: "rose",
                  value: analysis?.categorized_items?.zombies?.length || 3,
                ...
                <div
                  ...
                  onClick={() =>
                    sendMessage(
                      `Muéstrame el listado y plan para mis productos ${cat.title}`,
                    )
                  }
                  className="bg-[#0b0f19] p-4 rounded-2xl flex items-center justify-between group cursor-pointer hover:border-violet-500/50 border border-transparent transition-all"
                >
          ...
          <button
            onClick={() => sendMessage("Dame un resumen de mi Portfolio")}
            className="w-full mt-6 py-3 bg-white/5 hover:bg-white/10 rounded-xl text-xs font-bold transition-all text-slate-300"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 relative z-10 mb-20 lg:mb-0">
        <div className="bg-[#131826] p-8 rounded-3xl border border-white/5 flex flex-col">
          <h3 className="text-sm font-black text-slate-400 uppercase tracking-widest flex items-center gap-2 mb-6">
            <Target className="w-5 h-5 text-violet-500" /> Mis Objetivos
            (Configuración AI)
          </h3>
          <p className="text-xs text-slate-500 mb-4">
            Define aquí tus metas personales. Vanguard IA usará esta información
            como pilar base para trazar tu Plan Estratégico en el Chat de
            Comando.
          </p>
          <textarea
            value={goals}
          ...
          <div className="flex justify-between items-center">
            <button
              onClick={() => {
                if (confirm("¿Seguro quieres borrar tus objetivos actuales?")) {
                  setGoals("");
                  setIsGoalsSaved(false);
                }
              }}
        ...
        <div className="bg-[#131826] p-8 rounded-3xl border border-white/5 flex flex-col group relative overflow-hidden">
          ...
          <h3 className="text-sm font-black text-slate-400 uppercase tracking-widest flex items-center gap-2 mb-6">
            <ShieldCheck className="w-5 h-5 text-violet-500" /> Caja Negra
            (Memoria Estratégica)
          </h3>

          <p className="text-xs text-slate-500 mb-4">
            Hemisferio de memoria persistente. Guarda aquí planes, capturas de
            estrategias o datos útiles que Vanguard debe recordar siempre.
          </p>

          <textarea
            value={blackBox}
            onChange={(e) => {
              setBlackBox(e.target.value);
              setIsBlackBoxSaved(false);
            }}
            onPaste={(e) => {
              ...
              items.forEach((item) => {
                if (item.type.includes("image")) {
                  toast.info(
                    "Para compartir imágenes con Vanguard, usa el clip del Chat. Aquí guardamos el plan estratégico.",
                  );
                }
              });
            }}
          ...
          <div className="flex justify-between items-center mt-auto">
            <button
              onClick={() => {
                if (
                  confirm(
                    "¿Seguro quieres vaciar la Caja Negra? Esta acción es irreversible.",
                  )
                ) {
                  setBlackBox("");
                  handleSaveBlackBox();
            ...
            <div className="flex items-center gap-3">
              {!isBlackBoxSaved && (
                <span className="text-[10px] font-black text-violet-400 animate-pulse uppercase">
                  Memoria por anclar
                </span>
              )}
              <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 relative z-10 mt-6 mb-20 lg:mb-8">
        <div className="bg-[#131826] p-6 rounded-3xl border border-white/5">
          <h3 className="text-sm font-black text-slate-400 uppercase tracking-widest flex items-center gap-2 mb-6">
            <ShieldCheck className="w-5 h-5 text-emerald-500" /> Monitor Mercado
            Ads
          </h3>
          <div className="space-y-3">
            {(analysis?.ads_manager?.active_campaigns || []).map(
              (camp: any, i: number) => (
                <div
                  key={i}
                  className="flex items-center justify-between p-3 rounded-2xl bg-[#0b0f19] border border-emerald-500/10"
                >
                  <div className="flex flex-col w-48 sm:w-64">
                    ...
                    <span className="text-[10px] text-slate-500">
                      ...
                      | Objetivo ROAS: {camp.roas_target}
                    </span>
                  </div>
                  <div className="text-right shrink-0">
                    <div className="text-sm font-black text-emerald-400">
                      ${camp.budget.toLocaleString()} ARS
                    </div>
            ...
            {(!analysis?.ads_manager ||
              !analysis.ads_manager.active_campaigns ||
              analysis.ads_manager.active_campaigns.length === 0) && (
        ...
        <div className="bg-[#131826] p-6 rounded-3xl border border-white/5">
          <h3 className="text-sm font-black text-slate-400 uppercase tracking-widest flex items-center gap-2 mb-6">
            <Zap className="w-5 h-5 text-cyan-400" /> Embudo Orgánico (Top 5
            Visitas)
          </h3>
          <div className="space-y-3">
            {(currentMetrics?.top_items || [])
              ...
              .map((item: any, i: number) => (
                <div
                  key={i}
                  className="flex items-center justify-between p-3 rounded-2xl bg-[#0b0f19] border border-white/5"
                >
                  <div className="flex flex-col">
                    <span className="text-xs font-bold text-white truncate w-48 sm:w-64">
                      {item.title}
                    </span>
                    <span className="text-[10px] text-slate-500">
                      Salud: {Math.round((item.health || 0) * 100)}%
                    </span>
                  </div>
            ...
            {(!currentMetrics?.top_items ||
              currentMetrics.top_items.length === 0) && (
              <p className="text-xs text-slate-500 italic">
                No hay datos de tráfico orgánico.
              </p>
            )}
          </div>
        </div>
      ...
      >
              ...
              {messages.length === 0 && (
                <div className="text-center py-10 opacity-50">
                  ...
                  <p className="text-xs font-bold text-slate-400">
                    Tu Partner Estratégico de IA está listo para operar.
                  </p>
                </div>
              )}
              {messages.map((msg, i) => {
                    ...
                    >
                        ...
                        <div className="grid grid-cols-2 gap-2 mb-2">
                          {msg.attachments.map((url: string, idx: number) => (
                            <img
                              ...
                              src={url}
                              alt={`Adjunto ${idx}`}
                              className="w-full rounded-lg h-32 object-cover border border-white/10"
                            />
                          ))}
                        </div>
                      )}
                      {msg.attachment && !msg.attachments && (
                        <img
                          src={msg.attachment}
                          alt="Adjunto"
                          className="w-full rounded-lg mb-2 max-h-40 object-cover border border-white/10"
                        />
                      )}
                      ...
                      {actionData && msg.role === "vanguard" && (
                        <div className="mt-3 p-3 bg-[#0b0f19] border border-emerald-500/30 rounded-xl shadow-[0_0_15px_rgba(16,185,129,0.1)]">
                          <div className="flex items-center gap-2 mb-2 text-emerald-400 font-bold text-[10px] uppercase tracking-wider">
                            <ShieldCheck size={14} /> Propuesta Ejecutiva
                            Protegida
                          </div>
                          ...
                          <div className="flex items-center gap-2">
                            <button
                              ...
                              className="flex-1 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold py-2 px-3 rounded-lg flex items-center justify-center gap-1 transition-colors"
                            >
                            ...
                            <button
                              ...
                              onClick={() =>
                                sendMessage(
                                  `[ADMINISTRADOR]: He DENEGADO tu propuesta de "${actionData.description}". Busca una alternativa.`,
                                )
                              }
                              className="bg-rose-900/40 hover:bg-rose-800 text-rose-300 border border-rose-500/30 text-xs font-bold py-2 px-3 rounded-lg transition-colors"
                            >
                              <X size={14} />
                            </button>
                          </div>
            ...
            <div className="p-4 bg-[#131826] border-t border-white/5">
                ...
                <div className="mb-3 flex gap-2 overflow-x-auto pb-2 custom-scrollbar">
                  {attachments.map((att, idx) => (
                    <div key={idx} className="relative shrink-0">
                      ...
                      />
                      <button
                        title="Remover imagen adjunta"
                        aria-label="Remover imagen adjunta"
                        onClick={() =>
                          setAttachments((prev) =>
                            prev.filter((_, i) => i !== idx),
                          )
                        }
                        className="absolute -top-2 -right-2 bg-slate-800 text-slate-300 rounded-full p-1 border border-white/10 hover:text-white transition-colors"
                      >
                        <X size={10} />
                      </button>
                    </div>
              ...
              <div className="relative flex items-center">
                ...
                <button
                  ...
                  className="absolute left-2 p-2 text-slate-400 hover:text-white transition-colors z-10 focus:outline-none"
                >
        ...
        >
          {isChatOpen ? (
            <X className="text-white w-7 h-7 group-hover:rotate-90 transition-transform" />
          ) : (
