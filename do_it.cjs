...
for (let i = 400; i < 600; i++) {
    ...
    if (start > -1 && i > start && lines[i] && lines[i].includes('                )}')) {
        ...
        // Don't break! Let it find the LAST one before it hits the Campos Dinámicos
    }
...
if (start > -1 && end > -1) {
    let blockLines = lines.splice(start, end - start + 1); // remove from array
    ...
    if (optStart > -1) {
        ...
        lines.splice(optStart - 2, 0, inject);
        fs.writeFileSync('components/ProductAdmin.tsx', lines.join('\n'));
        console.log('Successfully completed final migration step. Moved from ' + start + ' to ' + (optStart - 2));
    } else {
