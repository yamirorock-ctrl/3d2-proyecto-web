...
async function testUserProductModel() {
  // Get token
  ...
  console.log("Testing with NEW User Product Model (2025)...\n");

  const itemBody = {
    // NO TITLE - ML lo genera automáticamente
    family_name: "Juguete Personalizado 3D2",
    category_id: "MLA1910",
    price: 1500,
    currency_id: "ARS",
    ...
    description: {
      plain_text:
        "Producto de prueba para sincronización con el nuevo modelo UP.",
    },
    pictures: [
      {
        source:
  ...
  console.log("Payload (User Product Model):");
  ...
  if (response.ok) {
    console.log("\n✅ VALIDACIÓN EXITOSA - El nuevo modelo UP funciona!");
  } else if (data.cause) {
    console.log("\n=== DETAILED ERRORS ===");
...
testUserProductModel().catch(console.error);
