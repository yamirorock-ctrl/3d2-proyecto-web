...
async function cleanML() {
  ...
  const targets = products.filter((p) => {
    ...
    // Broaden search to include generic "Nombre" or "Personalizado" often used in errors
    return (
      txt.includes("bebé") ||
      txt.includes("bebe") ||
      txt.includes("colgante") ||
      txt.includes("puerta") ||
      txt.includes("nombre 3d")
    );
  });

  if (targets.length === 0) {
    console.log(
      "No matches found for keywords: bebe, colgante, puerta, nombre 3d",
    );
    return;
  }
